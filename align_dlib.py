...
class AlignDlib:
    ...
    def largestBoundingBoxesInFace(self, rgbImg, skipMulti=False):

        assert rgbImg is not None

        faces = self.allBoudingBoxesInFace(rgbImg)

        if (not skipMulti and len(faces) > 0) or len(faces) == 1:
            return max(faces, key=lambda rect: rect.width() * rect.height())
        else:
            return None
    def findLandmarksInFace(self, dimImg, rgbImg, bbox):

        assert rgbImg is not None
        assert bbox is not None

        landmarksPoins = self.predictor(rgbImg, bbox)
        return [(p.x, p.y) for p in landmarksPoints.parts()]

    def align(self, dimImg, rgbImg, bbox=None, landmarks=None, landmarkIndices=INNER_EYES_AND_BOTTOM_LIP,skipMulti=False, scale=1.0):

        assert dimImg is not None
        assert rgbImg is not None
